        ...
        db.collection('warehouse').doc('items').collection('available').where("sku", "==", item).get().then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                ...
                if(buynow.ship != undefined){
                    ...
                    `
                    <div id="info">
                        <h3 id="trycatch">Purchasing ${buynow.name}</h3>
                        <p>Color:</p>
                    </div>
                    <div id="flow" style="position: relative;">
                        ...
                        <select name="select-size" id="select-size"></select>
                        <br>
                        <h3>Shipping Info</h3>
                        <p>Recipient</p>
                        ...
                        <p>Total charged to card: $${titak}</p>
                        ...
                        <p style="color: crimson;">By continuing, you agree to allow Amazins PLC and it's parent company to process any data submitted through this form, as stated in our <a href="https://github.com/Amazins/store/blob/main/legal/privacy.md">Privacy Policy</a>.<br>Email is used to send shipping notifications, phone number is used for calling the residence if no one is present.<br><b>Orders to the EU, UK and Norway - VAT is applied.</b></p>
                    </div>
                    `
                } else {
                ...
                db.collection("warehouse").doc("filters").collection("typefilters").where("type", "==", itemType).get().then((querySnapshot) => {
                    querySnapshot.forEach((doc) => {
                        const tim = doc.data();
                        document.getElementById("select-size").innerHTML = tim.sizetable;
                    })
                })
            })
        })
